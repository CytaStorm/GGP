...
void Transformation::SetPosition(float a_x, float a_y, float a_z)
{
	if (a_x == m_position.x &&
		a_y == m_position.y &&
		a_z == m_position.z) {
...
void Transformation::SetPosition(DirectX::XMFLOAT3 a_position)
{	
...
void Transformation::SetRotation(float a_x, float a_y, float a_z, float a_w)
{
	if (a_x == m_rotation.x &&
		a_y == m_rotation.y &&
		a_z == m_rotation.z &&
...
void Transformation::SetRotation(DirectX::XMFLOAT4 a_rotation)
{
...
void Transformation::SetScale(float a_x, float a_y, float a_z)
{
	if (a_x == m_scale.x &&
		a_y == m_scale.y &&
		a_z == m_scale.z) {
...
void Transformation::SetScale(DirectX::XMFLOAT3 a_scale)
{
...
DirectX::XMFLOAT4 Transformation::GetRotation()
{
	return m_rotation;
}
...
DirectX::XMFLOAT4X4 Transformation::GetWorldInverseTransposeMatrix()
{
	return m_worldInverseTransposeMatrix;
}

void Transformation::MoveAbsolute(float a_x, float a_y, float a_z)
{
	...
	DirectX::XMVECTOR newPosition = DirectX::XMLoadFloat3(&DirectX::XMFLOAT3(a_x, a_y, a_z));
...
void Transformation::MoveAbsolute(DirectX::XMFLOAT3 a_offset)
{
	...
	//Only update when offset not zero
	DirectX::XMVECTOR originalPosition = DirectX::XMLoadFloat3(&m_position);
	DirectX::XMVECTOR newPosition = DirectX::XMVectorAdd(originalPosition, newPosition);

	DirectX::XMStoreFloat3(&m_position, newPosition);
	m_dirtied = true;
}

void Transformation::Rotate(float a_pitch, float a_yaw, float a_roll)
{
...
void Transformation::Rotate(DirectX::XMFLOAT4 a_rotation)
{
	if (a_rotation.x == 0 &&
		a_rotation.y == 0 &&
		a_rotation.z == 0 &&
...
void Transformation::Scale(float a_x, float a_y, float a_z)
{
	...
	DirectX::XMVECTOR newScale = DirectX::XMLoadFloat3(&DirectX::XMFLOAT3(a_x, a_y, a_z));
...
void Transformation::Scale(DirectX::XMFLOAT3 a_scale)
{
...
void Transformation::CalculateWorldMatrix()
{
	if (!m_dirtied) return;
